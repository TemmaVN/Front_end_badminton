...
export const ProductProvider = ({ children }) => {
    ...
    const filterProducts = useCallback(async (payload = {}) => {
        setLoading(true);
        setError(null);
        try {
            const cleanPayload = {
                ...payload,
                categorySlugs: payload.categorySlugs?.filter(Boolean) ?? [],
                brandSlugs: payload.brandSlugs?.filter(Boolean) ?? [],
                priceRanges: payload.priceRanges?.filter((r) => r?.min != null || r?.max != null) ?? [],
                page: payload.page || 1,
                pageSize: payload.pageSize || 10,
            };

            const response = await productApi.filter(cleanPayload);
