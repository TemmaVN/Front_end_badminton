...
function Card({ children, className = "" }) {
  return (
    <div
      className={`bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-2xl border border-slate-200/50 dark:border-slate-700/50 ${className}`}
    >
      {children}
    </div>
  );
...
function RevenueTab({ period }) {
  const { revenueByMonth, revenueByMonthLoading, revenueByMonthError } =
    useStatistic();
  const { isDark } = useTheme();
  const [monthly2026, setMonthly2026] = useState([]);
  const [loading2026, setLoading2026] = useState(false);
  const [error2026, setError2026] = useState(null);

  useEffect(() => {
    const fetchMonthly2026 = async () => {
      setLoading2026(true);
      setError2026(null);
  ...
  const validateAndTransform = (source) => {
  ...
  const validRevenueByMonth = validateAndTransform(revenueByMonth);
  const validMonthly2026 = validateAndTransform(monthly2026);

  const data = useMemo(() => {
    const src = period === "2025" ? validRevenueByMonth : validMonthly2026;
  ...
  }, [period, validRevenueByMonth, validMonthly2026]);

  const compareData = useMemo(() => {
    return MONTH_LABELS.map((label, idx) => {
      const r25 = validRevenueByMonth.find((r) => r.month === idx + 1);
      const r26 = validMonthly2026.find((r) => r.month === idx + 1);
      return {
        thang: label,
  ...
  }, [validRevenueByMonth, validMonthly2026]);

  const totalDT = data.reduce((s, d) => s + d.doanhThu, 0);
  const totalDH = data.reduce((s, d) => s + d.donHang, 0);

  // Kiểm tra có dữ liệu không
  const hasData = data.some((d) => d.doanhThu > 0 || d.donHang > 0);
  const isLoading = revenueByMonthLoading || loading2026;
  const hasError = revenueByMonthError || error2026;
    ...
    return (
      <Card className="p-12">
        <div className="text-center">
          <p className="text-red-500 font-semibold mb-2">❌ Lỗi tải dữ liệu</p>
      ...
      <Card className="p-12">
        <div className="text-center">
          ...
          <p className="text-xs text-slate-400">
            Chưa có đơn hàng hoặc dữ liệu chưa cập nhật cho năm {period}
          </p>
        </div>
      </Card>
    );
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                <Legend
                  formatter={(v) =>
                    v === "doanhThu" ? "Doanh thu (triệu đ)" : "Đơn hàng"
                  }
                />
                <Area
                  type="monotone"
        ...
        {/* Bar comparison 2025 vs 2026 */}
        <Card className="p-6">
          <h4 className="text-base font-bold text-slate-800 dark:text-white mb-1">
            So sánh năm 2025 – 2026
          </h4>
          <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">
            ...
            <thead>
              <tr className="border-b border-slate-200/50 dark:border-slate-700/50 bg-slate-50/50 dark:bg-slate-800/50">
                {["Tháng", "Doanh thu", "Đơn hàng", "TB/đơn"].map((h) => (
                  <th
                    key={h}
                    className={`p-4 font-semibold text-slate-600 dark:text-slate-400 ${h === "Tháng" ? "text-left" : "text-right"}`}
                  >
                    {h}
                  </th>
                ))}
            ...
            <tbody>
              ...
              <tr className="bg-orange-50/50 dark:bg-orange-900/10 font-bold border-t-2 border-orange-100 dark:border-orange-900/30">
                ...
                <td className="p-4 text-right text-orange-500">
                  {(totalDT * 1_000_000).toLocaleString("vi-VN")} đ
                </td>
                <td className="p-4 text-right text-slate-800 dark:text-white">
                  {totalDH.toLocaleString()}
                </td>
                <td className="p-4 text-right text-slate-500 dark:text-slate-400">
                  —
                </td>
...
function CategoryTab() {
  ...
  const stackedData = useMemo(() => {
    ...
    return MONTH_LABELS.map(
      (_, idx) => months[idx + 1] ?? { thang: MONTH_LABELS[idx] },
    );
  }, [catMonthList]);
              ...
              <div key={i} className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <span className="text-xs text-slate-400">
                    {Math.round(item.doanhThu / 1_000_000)}M
                  </span>
                  <span className="text-xs font-bold text-slate-800 dark:text-white w-10 text-right">
                    {item.value}%
                  </span>
                </div>
              </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-200/50 dark:border-slate-700/50 bg-slate-50/50 dark:bg-slate-800/50">
                ...
                <th className="text-right p-4 font-semibold text-slate-600 dark:text-slate-400">
                  Doanh thu
                </th>
                <th className="text-right p-4 font-semibold text-slate-600 dark:text-slate-400">
                  Đơn hàng
                ...
                <th className="p-4 font-semibold text-slate-600 dark:text-slate-400 hidden md:table-cell">
                  Phân bổ
                </th>
              </tr>
            </thead>
...
function BrandTab() {
  ...
  const maxDT = brandData.length
    ? Math.max(...brandData.map((b) => b.doanhThu))
    : 1;
        ...
        <div className="h-72">
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <Tooltip
                contentStyle={makeTooltipStyle(isDark)}
                formatter={(v) => [`${v} triệu đ`, "Doanh thu"]}
              />
            ...
            <div key={i} className="flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <span className="text-xs text-slate-400">
                    {brand.donHang.toLocaleString()} đơn
                  </span>
                </div>
                <div className="h-2 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${maxDT > 0 ? (brand.doanhThu / maxDT) * 100 : 0}%`,
    ...
    <Card className="overflow-hidden">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-slate-200/50 dark:border-slate-700/50 bg-slate-50/50 dark:bg-slate-800/50">
              <th className="text-center p-4 font-semibold text-slate-600 dark:text-slate-400 w-12">
                #
              </th>
              <th className="text-left p-4 font-semibold text-slate-600 dark:text-slate-400">
                Sản phẩm
              </th>
              <th className="text-left p-4 font-semibold text-slate-600 dark:text-slate-400 hidden md:table-cell">
                Danh mục
              </th>
              <th className="text-left p-4 font-semibold text-slate-600 dark:text-slate-400 hidden lg:table-cell">
                Thương hiệu
              </th>
              <th className="text-right p-4 font-semibold text-slate-600 dark:text-slate-400">
                Đã bán
              ...
              filtered.map((p, i) => (
                ...
                >
                  <td className="p-4 text-center">
                    <span
                      className={`inline-flex items-center justify-center w-7 h-7 rounded-full text-xs font-bold ${p.rank <= 3 ? "bg-orange-100 text-orange-600 dark:bg-orange-900/30" : "bg-slate-100 text-slate-500 dark:bg-slate-800"}`}
                    >
                      {p.rank}
                    </span>
                  </td>
                  ...
                  <td className="p-4 text-right">
                    <div className="flex items-center justify-end gap-1">
                      ...
                      <span
                        className={`text-xs font-semibold ${p.trend === "up" ? "text-emerald-500" : "text-rose-500"}`}
                      >
                        {p.change}
                      </span>
                    </div>
...
function OrdersTab() {
  ...
  const cancelledOrders =
    orderStatus.find((r) => r.statusId === 8)?.totalOrders ?? 0;
  const shippingOrders =
    orderStatus.find((r) => r.statusId === 5)?.totalOrders ?? 0;
  ...
  const donutData = STATUS_GROUPS.map((g) => ({
    name: g.label,
    value: orderStatus
      .filter((r) => g.ids.includes(r.statusId))
      .reduce((s, r) => s + r.totalOrders, 0),
    color: g.color,
  })).filter((d) => d.value > 0);
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          },
          {
            ...
            value: `${completedRate}%`,
            sub: `${completedOrders} đơn hoàn tất`,
            color: "text-emerald-500",
          },
          {
            label: "Tỉ lệ hủy đơn",
            value: `${cancelledRate}%`,
            sub: `${cancelledOrders} đơn đã hủy`,
            color: "text-rose-500",
          },
          {
            label: "Đơn đang giao",
            value: shippingOrders.toLocaleString(),
            sub: "Trạng thái đang giao hàng",
...
function VouchersTab() {
  ...
  const totalDiscount = data.reduce(
    (s, d) => s + (d.totalDiscount ?? d.totalDiscountAmount ?? 0),
    0,
  );
  const totalUsed = data.reduce(
    (s, d) => s + (d.timesUsed ?? d.totalUsed ?? 0),
    0,
  );
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        {[
          ...
          {
            label: "Tổng giảm giá",
            value: fmtRevenue(totalDiscount),
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {data.map((row, i) => {
                ...
                const discount =
                  row.totalDiscount ?? row.totalDiscountAmount ?? 0;
                return (
                  <tr
                    key={i}
                    className="border-b border-slate-100 dark:border-slate-800 hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors"
                  >
                    ...
                    <td className="p-4 text-right text-slate-500 dark:text-slate-400">
                      {used > 0
                        ? Math.round(discount / used).toLocaleString("vi-VN") +
                          " đ"
                        : "—"}
                    </td>
                  </tr>
...
function CustomersTab() {
  ...
  return (
    <Card className="overflow-hidden">
      <div className="p-6 border-b border-slate-200/50 dark:border-slate-700/50">
        <h4 className="text-base font-bold text-slate-800 dark:text-white">
          Khách hàng tiêu dùng nhiều nhất
        </h4>
        <p className="text-sm text-slate-500 dark:text-slate-400">
          Top {data.length} khách hàng
        </p>
      </div>
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
            ...
            {data.map((row, i) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="p-4 text-right text-slate-500 dark:text-slate-400">
                    {orders > 0
                      ? Math.round(spent / orders).toLocaleString("vi-VN") +
                        " đ"
                      : "—"}
                  </td>
                </tr>
...
export default function Statistics() {
  ...
  const renderTab = () => {
    switch (activeTab) {
      case "revenue":
        return <RevenueTab period={period} />;
      case "category":
        return <CategoryTab />;
      case "brand":
        return <BrandTab />;
    ...
    <div className="space-y-6 p-4 md:p-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {summaryCards.map((s, i) => (
          <div
            key={i}
            className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-2xl p-5 border border-slate-200/50 dark:border-slate-700/50"
          >
