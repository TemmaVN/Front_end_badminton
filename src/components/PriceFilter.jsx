...
const PriceFilter = ({rangePrice, setRangePrice}) => {
  ...
  useEffect(() => {
    if (rangePrice && (rangePrice[0] !== localRange[0] || rangePrice[1] !== localRange[1])) {
      setLocalRange(rangePrice);
    }
  }, [rangePrice]);
  return (
    <div className="w-full max-w-sm p-4 bg-white rounded-xl shadow-sm">
      <h3 className="text-[#f15a22] font-bold mb-8 text-lg">Hoặc chọn khoảng giá:</h3>
      ...
      <div className="flex items-center justify-between gap-3">
        <div className="relative flex items-center w-full">
          ...
          <div className="w-full pl-8 pr-6 py-2 border border-gray-200 rounded-lg text-sm text-right font-medium text-gray-700 bg-gray-50">
            {localRange[0].toLocaleString()} <span className="text-[10px] ml-1">đ</span>
          </div>
        </div>
        ...
        <div className="relative flex items-center w-full">
          ...
          <div className="w-full pl-8 pr-6 py-2 border border-gray-200 rounded-lg text-sm text-right font-medium text-gray-700 bg-gray-50">
            {localRange[1].toLocaleString()} <span className="text-[10px] ml-1">đ</span>
          </div>
        </div>
      </div>
    </div>
