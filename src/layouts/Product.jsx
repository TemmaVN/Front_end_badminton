...
import { File, FileDown, Folder, Search, FilterIcon, ChevronDown } from 'lucide-react';
            ...
            <div className='min-h-screen text-[#333] p-4'>
                ...
                <div className='container max-w-350 mx-auto px-4 py-8 '>
                    <div className={`flex grow ${isHighMediumScreen? '':'flex-col'} mb-20 items-center justify-between`}>
                        <h2 className={`font-bold whitespace-nowrap text-3xl px-8`}>Phân loại sản phẩm</h2>
                        <div className={`flex ${isHighMediumScreen? 'gap-6': 'gap-2'} ${isSmallScreen? 'flex-col': ''}`}>
                            <div className={`flex ${isSmallScreen? 'gap-4 py-1':''} ${isHighMediumScreen? 'gap-6': 'gap-2'} `}>
                                <Button variant="search" size="search" className={`whitespace-nowrap ${isHighMediumScreen ? 'py-3 px-5 text-1xl' : 'px-2 py-2 text-[18px]'} gap-2 flex items-center`}><FcFolder className='w-6 h-6'/> Sản phẩm Limited</Button>
                                <Button variant="search" size="search" className={`whitespace-nowrap ${isHighMediumScreen ? 'py-3 px-5 text-1xl' : 'px-2 py-2 text-[18px]'} gap-2 flex items-center`}><HiFire className='w-6 h-6 text-red-600'/> Sản phẩm bán chạy</Button>
                            </div>
                            <div className={`flex ${isSmallScreen? 'gap-4 py-1':''} ${isHighMediumScreen? 'gap-6': 'gap-2'} `}>
                                <Button variant="search" size="search" className={`whitespace-nowrap ${isHighMediumScreen ? 'py-3 px-5 text-1xl' : 'px-2 py-2 text-[18px]'} gap-2 flex items-center`}><img src='https://static.fbshop.vn/template/assets/images/icon-cate-new.png' className='w-6 h-6'/> Sản phẩm Limited</Button>
                                <Button variant="search" size="search" className={`whitespace-nowrap ${isHighMediumScreen ? 'py-3 px-5 text-1xl' : 'px-2 py-2 text-[18px]'} gap-2 flex items-center`}><img src='https://static.fbshop.vn/template/assets/images/icon-cate-tag.png' className='w-6 h-6'/> Sản phẩm Limited</Button>
                            </div>
                        </div>
                    </div>
                        ...
                        <div className='flex-1'>
                            <div className='flex flex-col justify-between mb-8 pb-4 border-b border-gray-100 gap-4'>
                                <div className='flex items-center gap-3'>
                                    ...
                                    <span className='text-xs text-gray-400 bg-gray-bg px-3 py-1 rounded-full'>0 sản phẩm</span>
                                </div>
                                <div className={`flex items-center justify-between gap-4 ${isSmallScreen? 'flex-col items-start': ''}`}>
                                    ...
                                    <div className='flex items-center gap-3 shrink-0'>
                                        ...
                                        <div className='relative'>
                                            <select className='appearance-none bg-gray-50 border border-gray-200 rounded-lg pl-4 pr-10 py-2 text-sm font-medium focus:ring-1 focus:ring-orange-default outline-none cursor-pointer text-gray-800' name="" id="">
                            ...
                            <div className="flex flex-col items-center justify-center pt-24 pb-32 text-center 
                                ...
                                <h3 className="text-xl font-bold text-gray-800">Chưa có sản phẩm nào trong danh sách</h3>
                            </div>
                        </div>
                    </div>
                </div>
